...
Full metrics: `evaluation/metrics\_report.json`
...
│   └── train\_reranker.py    # Cross-encoder reranker fine-tuning

├── evaluation/

│   ├── run\_evaluation.py    # Evaluation script (Recall@K, MRR@K, nDCG@K)

│   └── metrics\_report.json  # Baseline vs tuned comparison
...
python finetuning/train\_reranker.py
...
gcloud builds submit --tag gcr.io/YOUR\_PROJECT\_ID/rag-system
...
&nbsp; --image gcr.io/YOUR\_PROJECT\_ID/rag-system \\
...
&nbsp; --set-env-vars="DB\_HOST=YOUR\_DB\_HOST,PROJECT\_ID=YOUR\_PROJECT\_ID"
...
&nbsp;     "url": "https://en.wikipedia.org/wiki/Machine\_learning",
...
\- `evaluation/metrics\_report.json` — full metrics
